...
function setColor(event)
{
    /*
    ---------------------------------------------------
    == -  сравнивает два значени€;
    === - сравнивает два значени€ и типы этих значений;
    === - возвращает 'true' только в том случае, если совпадают как значени€, так и типы;
    -----------------------------------------------------
    */
...
function tickCountdown()
{
    ...
    date = date * SECONDS_PER_DAY;//”бираем врем€ дн€, полученное выше

    let years = Math.floor(date / SECONDS_PER_YEAR);
    if (years > 0) {
        date = date % SECONDS_PER_YEAR;//≈сли промежуток времени больше года, убираем годы,т.к. мы их уже получили
        //ѕолучаем блок, отображающий годы:
        let years_unit = document.getElementById("years-unit");
        //≈сли соответствующего блока нет на странице, то создаем его:
        if (years_unit == null) {
